...
<div class="showtimes view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        <tr>
            ...
            <td><?= $showtime->has('$movies') ? $this->Html->link($showtime->$movies->name, ['controller' => 'Movies', 'action' => 'view', $showtime->movie->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Room') ?></th>
            <td><?= $showtime->has('$rooms') ? $this->Html->link($showtime->$rooms->name, ['controller' => 'Rooms', 'action' => 'view', $showtime->room->id]) : '' ?></td>
        </tr>
        <tr>
